        ...
        return questions.length > 0 ? (
            <div className={style.container}>
                {currentIndex >= questions.length || finishGame ? (
                    <div className={style.winBlock}><h1 className={style.winTitle}>YOUR SCORE WAS {score}</h1>
                        <button className={style.btnHome} onClick={() => changeGame(3)}>HOME</button>
                    </div>) : (
                    <Questions handleFinishGame={handleFinishGame}
                        isActive={isActive}
                               startGame={startGame} changeGame={changeGame} handleNextQuestion={handleNextQuestion}
                               showAnswers={showAnswers}
                               questions={questions[currentIndex]} handleAnswer={handleAnswer}/>)}
            </div>) : <div>Loading...</div>
    }
;
